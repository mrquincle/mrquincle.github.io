...
$$
\theta_{n+1} | \theta_1 \ldots \theta_{n-1} \sim \frac{1}{\alpha+n} (\alpha G_0 + \sum_{i=1}^n \delta_{\theta_i})
$$

And just to write this down in a totally equivalent notation for your convenience:

$$
p(\theta_m|\theta_{-m},\alpha,G_0) = \frac{a}{M-1+\alpha} G_0 + \frac{1}{M-1+\alpha} \sum_{j=1,j\neq m}^M \delta_{\theta_j}
$$
...
The definition of $p(\theta_i\|\theta_{-i},y_i)$ leads to Gibbs sampling by just drawing from it for all indices $(i=1,\ldots,n)$. 

To remind you about the basics of Gibbs sampling; if we sample the conditionals $p(a_0\|a_1,b)$ and $p(a_1\|a_0,b)$ iteratively, we will get the distribution $p(a_0,a_1\|b)$ which is joint in $a_0$ and $a_1$. So, our sampling results in $p(\theta\|y)$ with $y$ all observations. Note that Gibbs sampling allows you to forget a value $\theta_i$ when it is time to sample it again.
...
$$
c_i | p \sim Discrete(p_1,\ldots,p_K) \\
\phi_c \sim G_0 \\
...
$$ p(c_1,\ldots,c_{i-1};\alpha) = \int p(c_1,\ldots,c_{i-1}|p) p(p|\alpha) dp $$

And:

$$ p(c_1,\ldots,c_{i-1},c_i=c;\alpha) = \int p(c_1,\ldots,c_{i-1},c_i=c|p) p(p|\alpha) dp $$
...
$$ p(c_i=c|c_1,\ldots,c_{i-1}) = \frac{p(c_1,\ldots,c_{i-1},c_i=c)}{p(c_1,\ldots,c_{i-1})}$$
...
$$ p(c_i=c|c_1,\ldots,c_{i-1}) = \frac{ 
 \frac{\Gamma(\alpha)}{\Gamma(\alpha + n)} \prod_k \frac{\Gamma(\alpha/K + n_k)}{\Gamma(\alpha/K)} }
 {\frac{\Gamma(\alpha)}{\Gamma(\alpha + n - 1)} \prod_k \frac{\Gamma(\alpha/K + n_{k,-i})}{\Gamma(\alpha/K)} }$$

And we are happy to see we can cross-out a lot of the terms in this fraction:

$$ p(c_i=c|c_1,\ldots,c_{i-1}) = \frac{\Gamma(\alpha + n - 1)}{ \Gamma(\alpha+n)}
...
$$ p(c_i=c|c_1,\ldots,c_{i-1}) = \frac{1}{\alpha+n-1}
\prod_k \frac{ \Gamma(\alpha/K + n_k)} 
{\Gamma(\alpha/K + n_{k,-i})}$$
...
The first time I encountered, just integrate out the mixing proportions $p_i$ I did not have a clue that a dozen steps
...
$$\sum_{\omega \in \Omega} p(c_i=\omega|c_1,\ldots,c_{i-1}) = 1$$

Apply this:

$$ p(c_i\neq c|c_1,\ldots,c_{i-1}) = 1 - \frac{\alpha/K + n_{c,-i}  }{\alpha+n-1} 
...
To sample a density $\pi(x)$ a proposal density, $g(x^\*\|x)$, is used in calculating an acceptance probability $a(x^*,x)$:
