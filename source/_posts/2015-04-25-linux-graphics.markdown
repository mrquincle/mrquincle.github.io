...
What do I actually have in my laptop? First of all, it is important to know what to search for. There are three basic
components that we need to know, my system consists of:
...
Note also that this 3rd Gen Intel Core family is called the Ivy Bridge (see also this 
...
the HM76 chipset. This is also known as the [BD82HM76 PCH](http://ark.intel.com/products/64345/Intel-BD82HM76-PCH) 
and known under *Panther Point*.  
...
The HD Graphics 4000 is a [Intel HD](http://en.wikipedia.org/wiki/Intel_HD_and_Iris_Graphics#Ivy_Bridge) variant 
...
The Direct Rendering Manager has a DRM core that is independent of the specific drivers required for all the hardware
that is available. It provides the interface to user-space code. A driver consists out of two parts, GEM [Graphics
...
Laurens Pinchart has a nice presentation on DRM, KMS, and in specific writing drivers at 
[YouTube](https://www.youtube.com/watch?v=Ja8fM7rTae4). The picture below if from his presentation.
...
has (reference to) a display mode, offsets into the video memory, etc. The controller links to an encoder (which can
be off-chip). If it links to multiple encoders, these will receive data from the same scanout buffer, so they will
display the same, cloned, data. The connectors, finally, can be only connected to one encoder, and knows how to talk
...
Startup with drm.debug=14 (or drm.debug=0x0e) and you will get plenty of debug information from the DRM. For example,
that we run Panther Point PCM is indeed confirmed in the syslog:
...
many display pipes are available (3 in my case). It enables `ENCODER:31:LVDS-31` and `CONNECTOR:30:LVDS-1`. There are
several connectors also to choose from:
...
Information at [virtuousgeek](http://virtuousgeek.org/blog/index.php/2011/01/) also explains a bit about power
management w.r.t. displays. For example memory can enter self-refresh in which it consumes way less power. The display
plane FIFO watermarks can be set conservatively, however, this leads to long periods in which self-refresh doesn't 
happen. It can be set aggressively, FIFO underruns can occur.

Now, we know what kind of hardware we have, let us first try to blindly apply the newest kernel from Intel...

# Trying the newest DRM kernel for Intel
...
wget kernel.ubuntu.com/~kernel-ppa/mainline/drm-intel-next/${kernel}/linux-image-${version}-generic_${subversion}_amd64.deb
wget kernel.ubuntu.com/~kernel-ppa/mainline/drm-intel-next/${kernel}/linux-headers-${version}_${subversion}_all.deb
wget kernel.ubuntu.com/~kernel-ppa/mainline/drm-intel-next/${kernel}/linux-headers-${version}-generic_${subversion}_amd64.deb
...
know. So, we go back to the default kernel (4.0.0-040000rc7-generic #201504142105). 
